...
class NicobotEnv(tower_env.TowerEnv):
    ...
    def _set_action(self, action):
        ...
        if self.gripper_relative_target:
            ...
            pos_ctrl[0] += 0.45  # add constant to x-axis to avoid generating actions behind the robot
            pos_ctrl[1] += 0.10
            pos_ctrl[2] += 0.6
    ...
    def _env_setup(self, initial_qpos):
        ...
        gripper_target = np.array([0.64, 0.1, 0.1 + self.gripper_extra_height]) + self.sim.data.get_site_xpos(
            'robot0:grip')
