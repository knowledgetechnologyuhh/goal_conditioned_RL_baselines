...
class Layer():
    def __init__(self, level, env, agent_params, device):
        ...
        self.buffer_size = min(self.trans_per_attempt * self.time_limit**(self.n_levels - 1 - self.level) *
        ...
        self.replay_buffer = ExperienceBuffer(self.buffer_size,
                                              ...
                                              env.state_dim, act_dim, goal_dim)
        ...
        self.noise_perc = agent_params["atomic_noise"] if self.level == 0 else agent_params["subgoal_noise"]
    ...
    def add_noise(self, action, env):
        ...
        action_bounds = env.action_bounds if self.level == 0 else env.subgoal_bounds_symmetric
        action_offset = env.action_offset if self.level == 0 else env.subgoal_bounds_offset
    ...
    def get_random_action(self, env):
        ...
        action = np.zeros((env.action_dim if self.level == 0 else env.subgoal_dim))
    ...
    def perform_action_replay(self, hindsight_action, next_state, goal_status):
        ...
        transition = np.array([
            self.current_state, hindsight_action, reward, next_state,
            self.goal, finished, None
        ])
        self.replay_buffer.add(transition)

    def create_prelim_goal_replay_trans(self, hindsight_action, next_state, env):
        ...
        transition = np.array([
            self.current_state, hindsight_action, None, next_state, None, None,
            hindsight_goal
        ])
    ...
    def finalize_goal_replay(self, goal_thresholds):
        ...
        for i in range(len(indices)):
            ...
            for index in range(num_trans):
                ...
                # Update reward
                trans_copy[index][2] = self.get_reward(new_goal,
                                                       trans_copy[index][6],
                                                       goal_thresholds)
    ...
    def penalize_subgoal(self, subgoal, next_state, test_fail=True):
        ...
        transition = np.array([
            self.current_state, subgoal,
            self.subgoal_penalty if test_fail else 0.0, next_state, self.goal,
            True, None
        ])
        self.replay_buffer.add(transition)

    # Determine whether layer is finished training
    def return_to_higher_level(self, max_lay_achieved, agent, env,
                               attempts_made):
        """
        ...
        elif agent.test_mode and self.level < self.n_levels - 1 and attempts_made >= self.time_limit:
            return True
    ...
    def train(self, agent, env, subgoal_test=False, episode_num=None, eval_data={}):
        ...
        while True:
            ...
            if self.level > 0:
                ...
                goal_status, eval_data, max_lay_achieved = agent.layers[self.level - 1].\
            ...
            if agent.verbose and goal_status[self.level]:
                ...
                print("\nEpisode %d, Layer %d, Attempt %d Goal Achieved" % (episode_num, self.level, attempts_made))
            ...
            if not agent.test_mode:
                ...
                self.create_prelim_goal_replay_trans(hindsight_action, agent.current_state, env)
                #
            ...
            if agent.verbose:
                print("\nEpisode %d, Level %d, Attempt %d" % (episode_num, self.level, attempts_made))
            ...
            if (max_lay_achieved is not None and max_lay_achieved >= self.level) or \
                ...
                if attempts_made >= self.time_limit and not goal_status[
                        self.level]:
                    self.maxed_out = True
    ...
    def learn(self, num_updates):
        ...
        for _ in range(num_updates):
            ...
            if self.fw:
                bonus = self.state_predictor.pred_bonus(
                    actions, old_states, new_states).unsqueeze(1)
            ...
            if self.fw:
                learn_history['fw_loss'].append(
                    self.state_predictor.update(old_states, actions,
                                                new_states))
        ...
        for reward_val in r_vals:
            learn_history["reward_{}_frac".format(reward_val)] = float(
                np.sum(np.isclose(learn_history['reward'], reward_val))) / len(learn_history['reward'])

        for k, v in learn_history.items():
            learn_summary[k] = v.mean() if isinstance(
                v, torch.Tensor) else np.mean(v)
