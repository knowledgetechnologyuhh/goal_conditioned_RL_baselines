...
class PDDL_Parser:
    ...
    def scan_tokens(self, filename_or_str):
        if os.path.isfile(filename_or_str):
            with open(filename_or_str,'r') as f:
                pddl_str = f.read()

        else:
            pddl_str = filename_or_str

        # Remove single line comments
        str = re.sub(r';.*$', '', pddl_str, flags=re.MULTILINE).lower()
        # Tokenize
        stack = []
        list = []
    ...
    def parse_domain(self, domain_filename_or_str):
        tokens = self.scan_tokens(domain_filename_or_str)
        ...
        # else:
        #     raise 'File does not match domain pattern'
    ...
    def parse_problem(self, problem_filename_or_str):
        tokens = self.scan_tokens(problem_filename_or_str)
        if type(tokens) is list and tokens.pop(0) == 'define':
