...
class RolloutWorker(Rollout):
    ...
    def __init__(self, make_env, policy, dims, logger, T, rollout_batch_size=1,
        ...
        print_summary(self.policy.FLAGS, self.policy.env)
        if not self.policy.FLAGS.test and not self.policy.FLAGS.train_only:
        ...
        self.n_epochs = self.policy.agent.FLAGS.n_epochs

        # Determine training mode.  If not testing and not solely training, interleave training and testing to track progress
        self.num_train_episodes = self.policy.agent.FLAGS.n_train_rollouts
        self.num_test_episodes = self.policy.agent.FLAGS.n_test_rollouts
    ...
    def generate_rollouts_update(self, n_episodes, n_train_batches):
        ...
        for batch in range(self.n_epochs):
            ...
            self.policy.agent.FLAGS.test = False
            ...
            for episode in tqdm(range(self.num_train_episodes)):
                ...
                if self.policy.agent.FLAGS.verbose:
                    print("\nBatch %d, Episode %d" % (batch, episode))

                # Train for an episode
                success, self.eval_data, train_duration = self.policy.agent.train(self.policy.env, episode, self.total_train_episodes, self.eval_data)
                ...
                self.total_train_steps += self.policy.agent.steps_taken

            # Save agent
            self.policy.agent.save_model(batch)
        ...
        updated_policy = self.policy.agent
    ...
    def test(self, batch, episode):
        ...
        self.policy.agent.FLAGS.test = True
        for episode in tqdm(range(self.num_test_episodes)):
            # Train for an episode
            success, self.eval_data, test_duration = self.policy.agent.train(self.policy.env,
            ...
            self.total_test_steps += self.policy.agent.steps_taken
        # Log performance
        success_rate = 0
        ...
        self.eval_data = self.policy.agent.prepare_eval_data_for_log(self.eval_data)
        self.policy.agent.log_performance(success_rate, self.eval_data,
                steps=self.total_train_steps, episode=self.total_train_episodes, batch=batch)
        print("\n--- END TESTING ---\n")
        early_stop_col = self.policy.FLAGS.early_stop_data_column
        if early_stop_col in self.eval_data.keys():
            early_stop_val = self.eval_data[early_stop_col]
            if self.policy.FLAGS.early_stop_threshold <= early_stop_val:
