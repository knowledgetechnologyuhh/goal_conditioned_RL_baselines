...
def display_var_info(vars):
    ...
    for v in vars:
        ...
        if "/b:" in name or "/biases" in name: continue    # Wx+b, bias is not interesting to look at => count _params, but not print
        logger.info("   %s%s %i _params %s" % (name, " "*(55-len(name)), v_params, str(v.shape)))
